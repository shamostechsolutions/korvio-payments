...
export async function POST(
  ...
  try {
    ...
    const updated = await createPledge({ campaign, contributor, amount });

    return NextResponse.json({
      contributor: contributorSummary(campaign, updated),
      message: "Pledge recorded",
    });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to record pledge" },
      { status: 400 },
    );
  }
}
